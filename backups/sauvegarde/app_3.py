    ...
    with tab_terrain:
        ...
        with _t_masques:
            ...
            if not proj_path:
                ...
                if not png_files:
                    ...
                    def _browse_mask_file(state_key):
                        try:
                            ...
                            path = filedialog.askopenfilename(
                                ...
                                filetypes=[("PNG 16-bit", "*.png"), ("Tous fichiers", "*.*")]
                            )
                            root.destroy()
                            if path:
                    ...
                    with col_out:
                        output_gaea_dir = st.text_input(
                            ...
                            key="gaea_output_dir_input",
                            placeholder="Chemin du dossier de sortie..."
                        )
                        if output_gaea_dir != st.session_state.get("gaea_output_dir", ""):
                            st.session_state.gaea_output_dir = normalize_path(output_gaea_dir)
                            save_config()
                    with col_out_btn:
                        st.markdown("<br>", unsafe_allow_html=True)
                        st.button("📂", key="browse_output_dir",
                                  ...
                                  help="Parcourir")
                    output_gaea_dir_clean = normalize_path(st.session_state.get("gaea_output_dir", default_out))
                    st.divider()
                    ...
                    if st.button("🔄 Générer masques corrigés", type="primary", disabled=not any_assigned):
                        with st.spinner("Traitement en cours..."):
                            try:
                                ...
                                # 2. Traiter chaque masque (flow + deposit)
                                mask_configs = [
                ...
                # Bouton vider cache
                proj_path_b = st.session_state.get('current_project_path')
                if proj_path_b:
                    cache_npz  = Path(proj_path_b) / "cache" / "terrain_data.npz"
                    cache_meta = Path(proj_path_b) / "cache" / "terrain_meta.json"
                    cache_exists = cache_npz.exists() or cache_meta.exists()
                    col_cache1, col_cache2 = st.columns([3, 1])
                    with col_cache1:
                        if cache_exists:
                            st.caption(f"Cache présent : `{cache_npz.name}`")
                        else:
                            st.caption("Aucun cache terrain détecté.")
                    with col_cache2:
                        if st.button("🗑️ Vider le cache", disabled=not cache_exists,
                                     help="Supprime terrain_data.npz — forcera un recalcul complet au prochain chargement"):
                            try:
                                if cache_npz.exists():  cache_npz.unlink()
