...
THRESHOLD_ROCK   = None   # p90 — rock
THRESHOLD_CLIFF  = None   # p95 — falaise
...
# ============================================================================
GAEA_MASKS = [
    (Path(r"H:\logiciel perso\Map generator\data\projects\Zimnitrita\masks\flow.png"),    "mask_flow"),
    (Path(r"H:\logiciel perso\Map generator\data\projects\Zimnitrita\masks\deposit.png"), "mask_deposit"),
...
def generate_slope_masks(slope: np.ndarray, t: dict,
    ...
    # Plateau : landes → rock
    # Descente : rock → rock+TRANS
    landes = np.zeros((out_size, out_size), dtype=np.float32)

    # Montée
    landes += smooth_transition(slope, t['gentle'], t['landes']) * \
              (1 - smooth_transition(slope, t['landes'], t['landes']))
    # Plateau entre landes et rock
    plateau = (slope >= t['landes']) & (slope < t['rock'])
    landes[plateau] = 1.0
    # Descente après rock
    fall_hi = t['rock'] + TRANSITION_WIDTH
    fall = (slope >= t['rock']) & (slope < fall_hi)
    landes[fall] = 1.0 - (slope[fall] - t['rock']) / TRANSITION_WIDTH
    landes[slope >= fall_hi] = 0.0

    # Recalcul propre
    landes = np.zeros((out_size, out_size), dtype=np.float32)
    LANDES_FLOOR = 0.45  # plancher dans les zones rock — à ajuster selon tests
    # Montée gentle → landes
    m_rise = (slope >= t['gentle']) & (slope < t['landes'])
    landes[m_rise] = (slope[m_rise] - t['gentle']) / (t['landes'] - t['gentle'])
    # Plateau landes → rock
    m_plat = (slope >= t['landes']) & (slope < t['rock'])
    landes[m_plat] = 1.0
    # Descente rock → rock+TRANS avec plancher
    fall_hi = t['rock'] + TRANSITION_WIDTH
    m_fall = (slope >= t['rock']) & (slope < fall_hi)
    landes[m_fall] = LANDES_FLOOR + (1.0 - LANDES_FLOOR) * (1.0 - (slope[m_fall] - t['rock']) / TRANSITION_WIDTH)
    landes[slope >= fall_hi] = LANDES_FLOOR  # plancher dans les zones rock pur
    ...
    # --- ROCK ---
    # Montée : landes → cliff (transition progressive)
    # Plateau : cliff et au-dessus
    rock = np.zeros((out_size, out_size), dtype=np.float32)
    # Montée depuis landes vers cliff
    m_rise_r = (slope >= t['landes']) & (slope < t['cliff'])
    rock[m_rise_r] = (slope[m_rise_r] - t['landes']) / (t['cliff'] - t['landes'])
    # Plateau au-dessus de cliff
...
def process_gaea_masks(gaea_masks: list, exclusion: np.ndarray,
    ...
    for src_path, out_name in gaea_masks:
