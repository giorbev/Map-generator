...
def check_qtre(output_dir: Path, priority: list, cellsize: float):
    ...
    # --- Visualisation ---
    _render_qtre_map(density_map, masks, priority, bloc_px, output_dir, verdict)


def _render_qtre_map(density_map: np.ndarray, masks: dict, priority: list,
                     bloc_px: int, output_dir: Path, verdict: str):
    """
    Génère qtre_map.png :
      - Heatmap densité par bloc (noir→vert→orange→rouge)
      - Blocs critiques encadrés en rouge vif
      - Légende masques actifs par couleur
    """
    n_blocs_y, n_blocs_x = density_map.shape
    H = n_blocs_y * bloc_px
    W = n_blocs_x * bloc_px

    # Palette : 0=noir, 1-3=vert, 4-5=orange, 6+=rouge
    PALETTE = {
        0: (20,  20,  20),
        1: (30, 120,  30),
        2: (60, 160,  60),
    ...
    # Image de base : heatmap densité
    img = np.zeros((H, W, 3), dtype=np.uint8)
    for by in range(n_blocs_y):
        for bx in range(n_blocs_x):
            d = int(density_map[by, bx])
            color = PALETTE.get(min(d, 8), (255, 0, 0))
            y0, x0 = by * bloc_px, bx * bloc_px
            img[y0:y0+bloc_px, x0:x0+bloc_px] = color

    # Encadrer les blocs critiques (6+) en rouge vif
    for by in range(n_blocs_y):
        for bx in range(n_blocs_x):
            if density_map[by, bx] >= 6:
                y0, x0 = by * bloc_px, bx * bloc_px
                y1, x1 = y0 + bloc_px - 1, x0 + bloc_px - 1
                cv2.rectangle(img, (x0, y0), (x1, y1), (0, 0, 255), 1)

    # Overlay masques actifs — teinte par masque
    MASK_COLORS = {
        "mask_seabed":           (80, 120, 200),
        "mask_coastal_flat":     (180, 160, 100),
        "mask_coastal_slope":    (140, 100,  60),
        "mask_flow":             (60,  60, 160),
        "mask_deposit":          (100,  80,  40),
        "mask_landes_rocheuses": (120, 100,  80),
        "mask_rock":             (100,  90,  90),
        "mask_prairie_humide":   (80, 180, 100),
        "mask_prairie_seche":    (160, 170,  80),
        "mask_landes_plateau":   (140, 110,  70),
        "mask_maquis_landes":    (110, 100,  60),
        "mask_alpages":          (180, 200, 140),
        "mask_foret_feuillue":   (30,  90,  30),
        "mask_foret_coniferes":  (20,  70,  50),
    }
    ...
    # Fond légende
    leg_h = 20 + len(MASK_COLORS) * 14 + 80
    cv2.rectangle(img, (legend_x - 4, legend_y - 4),
                  (legend_x + 160, legend_y + leg_h), (30, 30, 30), -1)

    # Titre
    cv2.putText(img, f"QTRE — {verdict}", (legend_x, legend_y + 12),
                font, 0.45, (255, 255, 255), 1, cv2.LINE_AA)
    legend_y += 20

    # Entrées palette densité
    for label, color in [("0  (vide)", (20,20,20)),
                          ("1-3 (OK)", (60,160,60)),
                          ("4-5 (limite)", (220,100,0)),
                          ("6+ (critique)", (200,20,20))]:
        cv2.rectangle(img, (legend_x, legend_y),
                      (legend_x + 10, legend_y + 10), color[::-1], -1)
        cv2.putText(img, label, (legend_x + 14, legend_y + 9),
                    font, fs, (220, 220, 220), ft, cv2.LINE_AA)
        legend_y += 13

    legend_y += 5
    cv2.putText(img, "Masques :", (legend_x, legend_y + 9),
    ...
    for name in priority:
        if name not in masks:
            continue
        color = MASK_COLORS.get(name, (180, 180, 180))
        short = name.replace("mask_", "")[:18]
        cv2.rectangle(img, (legend_x, legend_y),
                      (legend_x + 10, legend_y + 10), color[::-1], -1)
        cv2.putText(img, short, (legend_x + 14, legend_y + 9),
                    font, fs, (200, 200, 200), ft, cv2.LINE_AA)
        legend_y += 13

    out_path = output_dir / "qtre_map.png"
    cv2.imwrite(str(out_path), img)
    print(f"\n  [VIZ] qtre_map.png sauvegardé → {out_path}")
