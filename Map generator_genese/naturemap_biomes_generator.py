...
class NatureMapBiomesGenerator:
    ...
    # Palettes biomes GOOGLE MAPS STYLE (BGR - format OpenCV)
    COLOR_WATER = np.array([232, 245, 233], dtype=np.uint8)        # Eau: Bleu Google Maps léger
    COLOR_SNOW = np.array([255, 248, 240], dtype=np.uint8)       # Blanc Cassé #F0F8FF (BGR)
    COLOR_ROCK = np.array([180, 160, 140], dtype=np.uint8)       # Marron clair (pas de gris)
    COLOR_TUNDRA = np.array([150, 180, 150], dtype=np.uint8)     # Vert moyen olive
    COLOR_FOREST = np.array([100, 140, 90], dtype=np.uint8)      # Vert foncé naturel
    COLOR_PRAIRIE = np.array([170, 200, 140], dtype=np.uint8)    # Vert prairie clair
    COLOR_SAND = np.array([180, 200, 170], dtype=np.uint8)       # Vert-beige côtier
    ...
    def _compute_water_mask(self):
        ...
        # Seuil bas volontairement serré: quelques mètres au-dessus du minimum
        # ou percentile 2% des pixels valides, le plus conservateur des deux.
