...
def find_texture_file(textures_root: Path, middle_bcr: str) -> Optional[Path]:
    ...
    # Toutes les textures sont dans texture_middle/textures/ (vanilla + custom regroupés)
...
def apply_tint_to_texture(texture: np.ndarray, tint_rgb: Tuple[int, int, int]) -> np.ndarray:
    """
    Applique un tint à une texture — méthode TilW exacte.
    texture: (H, W, 3) uint8
    tint_rgb: (R, G, B) en sRGB [0-255]
    """
    # TilW : color = linear_to_srgb(MiddleColor × Color) en [0,1]
    # puis layer_np *= color (layer_np est en float32 0-255)
    color = np.array(tint_rgb, dtype=np.float32) / 255.0  # [0,1]
    result = texture.astype(np.float32) * color[None, None, :]
    return np.clip(result, 0, 255).astype(np.uint8)


def tile_texture(texture: np.ndarray, target_h: int, target_w: int,
                 tile_size_meters: float, pixels_per_meter: float = 1.0) -> np.ndarray:
    """
    Tile une texture sur une zone cible — méthode TilW exacte.
    TilW : resize texture à (MiddleScaleUV, MiddleScaleUV) pixels
    puis tile sur tout le terrain (1px = 1m).
    """
    from PIL import Image as PILImage

    # TilW resize à tile_size×tile_size pixels
    tile_px = max(1, round(tile_size_meters))
    tile = PILImage.fromarray(texture).resize((tile_px, tile_px), PILImage.BILINEAR)
    tile_np = np.array(tile)

    # Tile sur la zone cible
    result = np.zeros((target_h, target_w, 3), dtype=np.uint8)
    for y in range(0, target_h, tile_px):
        for x in range(0, target_w, tile_px):
            h = min(tile_px, target_h - y)
            w = min(tile_px, target_w - x)
            result[y:y+h, x:x+w] = tile_np[:h, :w]
...
def generate_tile_satmap_textured(
    ...
    for by in range(4):
        for bx in range(4):
            ...
            # Matériau de base w0 — couleur plate depuis catalogue
...
def generate_satmap_v2_textured_complete(
    ...
    # Stats textures
    found = len([k for k,v in texture_cache.items() if v is not None])
    not_found = len([k for k,v in texture_cache.items() if v is None])
    log(f"Textures chargées : {found} OK, {not_found} introuvables")
    if not_found > 0:
        missing_tex = [k for k,v in texture_cache.items() if v is None]
        log(f"Introuvables : {missing_tex[:10]}")
