...
# ============================================================
# RUN_CLASSIFICATION — Point d'entrée non-interactif
# ============================================================
def run_classification(
    ...
    n_clusters=20,
    classif_json=None,
    save_classif_json=None,
    interactive=False
):
    """
    Point d'entrée non-interactif pour intégration Streamlit.

    Args:
        input_path: Chemin satmap source (Path ou str). Ignoré si classif_json existe.
        output_dir: Dossier de sortie pour masks (Path ou str)
        n_clusters: Nombre de clusters K-means (défaut 20)
        classif_json: Si fourni et existe, charge classification depuis JSON (skip K-means)
        save_classif_json: Si fourni, sauvegarde classification après K-means
        interactive: Si True, appelle interactive_classify() (défaut False)

    Returns:
        dict {
            "masks_generated": int,
            "classif_path": str ou None,
            "n_clusters_used": int
        }
    """
    ...
    # ── CHARGEMENT CLASSIFICATION EXISTANTE ────────────────────────────────
    if classif_json and Path(classif_json).exists():
        print(f"\n📂 Chargement classification depuis {classif_json}...")
        with open(classif_json) as f:
            saved = json.load(f)

        centers = np.array(saved["centers"], dtype=np.uint8)
        classes_map = {int(k): v for k, v in saved["classification"].items()}
        n_clusters_used = saved["clusters"]

        # Recharger image source pour upscale
        if not input_path:
            input_path = saved.get("input")
        if not input_path or not Path(input_path).exists():
            raise ValueError(f"Image source introuvable : {input_path}")

        img = Image.open(input_path).convert("RGB")
        W, H = img.size

        # Recalculer labels_small pour upscale
        Ws = max(100, int(W * SCALE))
        Hs = max(100, int(H * SCALE))
        small = np.array(img.resize((Ws, Hs), Image.LANCZOS))

        # Réassigner labels via K-means sur la petite image
        pixels = small.reshape(-1, 3).astype(np.float32)
        dists = np.sqrt(((pixels[:, None, :] - centers[None, :, :]) ** 2).sum(axis=2))
        labels_small = np.argmin(dists, axis=1).reshape(small.shape[:2])

        print(f"   ✅ Classification chargée : {n_clusters_used} clusters")
    ...
    # ── NOUVEAU K-MEANS ─────────────────────────────────────────────────────
        ...
        print(f"   📸 apercu_kmeans.jpg")

        # Classification
        if interactive:
            classes_map = interactive_classify(centers, counts, order)
        else:
            # Mode auto : utiliser auto_classify() sur chaque centre
            print(f"\n🤖 Classification automatique...")
            classes_map = {}
            for i in range(len(centers)):
                r, g, b = centers[i]
                classes_map[i] = auto_classify(r, g, b)
            print(f"   ✅ {len(classes_map)} clusters classifiés automatiquement")
    ...
    classif_path = None
    if save_classif_json:
        save = {
            "input": str(input_path),
            "clusters": n_clusters_used,
            "classification": {str(k): v for k, v in classes_map.items()},
            "centers": centers.tolist()
        }
        with open(save_classif_json, "w") as f:
            json.dump(save, f, indent=2)
        classif_path = str(save_classif_json)
        print(f"\n✅ Classification sauvée → {Path(save_classif_json).name}")

    # ── RÉSULTAT ────────────────────────────────────────────────────────────
    masks_generated = len([f for f in output_dir.glob("mask_*.png")])

    return {
        "masks_generated": masks_generated,
        "classif_path": classif_path,
