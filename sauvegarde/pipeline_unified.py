...
def module_masques_base(dem: np.ndarray, terrain: Dict) -> Dict[str, np.ndarray]:
    ...
    if deposit is not None:
        ...
        active = deposit[deposit > 0]
        if active.size > 0:
            threshold = np.percentile(active, DEPOSIT_CUT_LOW * 100)
            deposit[deposit < threshold] = 0
        masques['mask_deposit'] = apply_output_curve(deposit, gamma=1.0)

    print(f"       Masques de base : seabed, coastal (80m), landes, rock, Gaea ({len(masques)} actifs)")
...
def module_vegetation(dem: np.ndarray, terrain: Dict) -> Dict[str, np.ndarray]:
    ...
    prairie_humide = np.clip((100 - dem) / 100, 0, 1) * np.clip((10 - slope) / 10, 0, 1)
    masques['mask_prairie_humide'] = prairie_humide

    # Prairie sèche (zones moyennes, pente douce)
    prairie_seche = np.clip((dem - 50) / 50, 0, 1) * np.clip((15 - slope) / 15, 0, 1)
    ...
    maquis_landes = np.clip((dem - 100) / 40, 0, 1) * np.clip((15 - slope) / 15, 0, 1) * 0.5
    ...
    foret_feuillue = np.clip((dem - 30) / 40, 0, 1) * np.clip((18 - slope) / 18, 0, 1)
