...
def compute_chunk_blends(h_chunk, s_chunk, c_chunk, sed_chunk, min_alt, alt_range, zones):
    ...
    # B. CÔTE PLATE
    # Boost BeachGrass sur vraies plages basses (0-10m) pour survivre au squeezing
    ...
    # D2. VALLÉES / CREUX (accumulation sédiments)
    sc["Dirt_03"]          += valley * (1.0 - steep) * 0.40  # Dirt dominant fond vallée plate
    sc["Dirt_01"]          += valley * (1.0 - steep) * 0.25
    sc["Grass_02"]         += valley * flat * 0.20           # Herbe si plat
    sc["Debris_Rock_01"]   += valley * moderate * 0.18       # Débris sur pente modérée

    # D3. CREUX ROCHEUX (accumulation débris/dirt entre rochers)
    sc["Dirt_03"]          += valley * steep * 0.28          # Dirt accumulé fond creux rocheux (remplacé Dirt_02)
    sc["Debris_Rock_01"]   += valley * steep * 0.35          # Debris déboulé des parois
    sc["Dirt_03"]          += valley * steep * wet * 0.20    # Dirt humide fond ravine rocheuse (cumul)
    ...
    sc["Dirt_01"]          += crest * (1.0 - wet)          * 0.18  # Remplacé Dirt_02 par Dirt_01
    sc["MountainGrass_01"] += crest * dry                  * 0.14  # Boost herbe alpine
    sc["Grass_01"]         += crest * (1.0 - steep) * (1.0 - wet) * 0.20  # Boost herbe

    # G. ALPAGES SECS
    sc["Heather_01"]       += alpage_dry                   * 0.34
    sc["MountainGrass_01"] += alpage_dry                   * 0.30  # Boost
    sc["Grass_01"]         += alpage_dry                   * 0.18  # Boost
    sc["Dirt_01"]          += alpage_dry                   * 0.14  # Remplacé Dirt_02 par Dirt_01

    # H. ALPAGES HUMIDES
    sc["MountainGrass_03"] += alpage_wet * moist           * 0.48  # Boost (remplace MG_02)
    sc["MountainGrass_01"] += alpage_wet                   * 0.28  # Remplacé MG_02 par MG_01 (tempéré)
    sc["Grass_03"]         += alpage_wet * moist           * 0.16  # Boost
    sc["Dirt_03"]          += alpage_wet * wet             * 0.15

    # I. PRAIRIES BASSES
    sc["Grass_01"]         += prairie_low * dry            * 0.30  # Boost
    sc["Grass_02"]         += prairie_low * dry            * 0.40  # Boost
    sc["Dirt_01"]          += prairie_low * dry            * 0.24  # Boost (remplace Dirt_02)
    # Dirt_02 supprimé de prairie_low
    ...
    sc["MountainGrass_01"] += prairie_mid * dry            * 0.38  # Remplacé MG_02 par MG_01 (tempéré)
    sc["MountainGrass_03"] += prairie_mid * moist          * 0.36  # Boost
    sc["Grass_02"]         += prairie_mid * dry            * 0.24  # Boost
    sc["Grass_03"]         += prairie_mid * moist          * 0.24  # Boost
    sc["Dirt_01"]          += prairie_mid * dry            * 0.20  # Ajusté
    # Dirt_02 supprimé, MountainGrass_02 remplacé par MG_01
    sc["Heather_01"]       += prairie_mid * convex * dry   * 0.14

    # K. PENTES MODÉRÉES
    sc["Grass_01"]         += mid_slope * dry              * 0.24  # Boost herbe
    sc["MountainGrass_01"] += mid_slope * (1.0 - moist)   * 0.28  # Boost herbe alpine
    sc["Dirt_01"]          += mid_slope * dry              * 0.18  # Remplacé Dirt_02 par Dirt_01
    ...
    sc["Rock_01"]          += rocky_highland               * 0.54  # Boost
    sc["Debris_Rock_01"]   += rocky_highland               * 0.26  # Boost
    sc["MountainGrass_01"] += rocky_highland * (1.0 - steep) * 0.12  # Boost (remplace Dirt_02)
    # Dirt_02 supprimé de rocky_highland
    ...
    # Nouveau : Affleurement rocheux côtier (bosse convexe)
