...
def calculate_flow_accumulation(heightmap, cellsize):
    ...
    # Gamma 1.5 avant blur : accentuer les chenaux forts
    flow = np.power(flow, 1.5).astype(np.float32)

    # Blur sigma=3 : fusionner les micro-chenaux D8, élargir les chenaux principaux
    flow = gaussian_filter(flow, sigma=3.0).astype(np.float32)
    ...
    # Cut-low 0.1 : supprimer le bruit de fond post-blur
    flow = np.where(flow < 0.1, 0.0, flow).astype(np.float32)
...
def calculate_deposit(heightmap, flow_raw, cellsize):
    ...
    # Blur pour lisser les contours (dépôts diffus, ~30m)
    deposit = gaussian_filter(deposit, sigma=max(1, int(30.0 / cellsize))).astype(np.float32)

    # Cut-low 0.5 : ne garder que les zones d'accumulation marquées
    deposit = np.where(deposit < 0.5, 0.0, deposit).astype(np.float32)
