...
# --- Masques Gaea ---
# Format : (chemin_source, nom_sortie, gamma)
GAEA_MASKS = [
...
def apply_output_curve(mask: np.ndarray, gamma: float = 1.0) -> np.ndarray:
    """Post-processing universel : stretch percentile → gamma → weight_min."""
    if STRETCH_AUTO:
        active = mask[mask > 0]
        if active.size > 0:
...
def process_gaea_masks(gaea_masks: list, exclusion: np.ndarray,
    ...
    for entry in gaea_masks:
        ...
        print(f"       gamma={gamma}  stretch={STRETCH_AUTO}  weight_min={WEIGHT_MIN}")
