...
Paramètre optionnel:
  --threshold 0.5          : Seuil personnalisé (défaut 0.3 soit ~1%)
...
def write_layer_dds(layer_path: Path, pixels: np.ndarray) -> bool:
    ...
    try:
        ...
        # Lire le header DDS pour trouver l'offset exact du mip0
        with open(layer_path, 'rb') as f:
            raw_header = f.read(4)  # magic 'DDS '
            if raw_header != b'DDS ':
                return False
            header_size_field = struct.unpack_from('<I', f.read(4))[0]  # dw Size = 124

            # Vérifier présence DX10 header (+20 bytes si FourCC='DX10')
            f.seek(84)
            fourcc = f.read(4)

        base_offset = 4 + header_size_field  # 4 (magic) + 124 = 128
        if fourcc == b'DX10':
            base_offset += 20  # DX10 extended header

        # Écrire uniquement le mip0 en place (seek + write, sans toucher au reste)
