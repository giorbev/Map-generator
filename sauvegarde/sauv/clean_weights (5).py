...
def write_layer_dds(layer_path: Path, pixels: np.ndarray) -> bool:
    ...
    try:
        ...
        # Lire le header existant en entier (taille = total - données pixel)
        file_size = layer_path.stat().st_size
        pixel_data_size = 512 * 512 * 4  # R32_UINT
        header_size = file_size - pixel_data_size

        with open(layer_path, 'rb') as f:
            dds_header = f.read(header_size)

        # Réécrire en préservant le header exact
